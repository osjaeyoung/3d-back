...
def run_1_cameraInit(binPath,baseDir,imgDir):
    ...
    subProc = subprocess.Popen(cmdLine, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True, universal_newlines=True)
    print(subProc.communicate())
    return 0
...
def run_2_featureExtraction(binPath,baseDir , numberOfImages , imagesPerGroup=40):
    ...
    if(numberOfImages>imagesPerGroup):
        ...
        print(cmdLine)
        subProc = subprocess.Popen(cmdLine, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
        print(subProc.communicate())
